...
impl SimpleSQLiteStorage {
    ...
    // 获取所有任务
    pub async fn get_all_tasks(app_handle: &AppHandle) -> Result<Vec<crate::BatchTask>, String> {
        let conn = Self::get_connection(app_handle)?;
        
        let mut stmt = conn.prepare("SELECT * FROM batch_tasks ORDER BY created_at DESC")
        ...
        let task_iter = stmt.query_map([], |row| {
            Ok(crate::BatchTask {
                ...
                config: serde_json::from_str(&row.get::<_, String>(11)?)
                    .map_err(|e| rusqlite::Error::ToSqlConversionFailure(Box::new(e)))?,
                items: serde_json::from_str(&row.get::<_, String>(12)?)
                    .map_err(|e| rusqlite::Error::ToSqlConversionFailure(Box::new(e)))?,
                results: serde_json::from_str(&row.get::<_, String>(13)?)
                    .map_err(|e| rusqlite::Error::ToSqlConversionFailure(Box::new(e)))?,
                error: row.get(14)?,
            })
    ...
    // 保存任务
    pub async fn save_task(app_handle: &AppHandle, task: &crate::BatchTask) -> Result<(), String> {
        let conn = Self::get_connection(app_handle)?;
    ...
    // 删除任务
    pub async fn delete_task(app_handle: &AppHandle, task_id: &str) -> Result<(), String> {
        let conn = Self::get_connection(app_handle)?;
    ...
    // 清空所有任务
    pub async fn clear_tasks(app_handle: &AppHandle) -> Result<(), String> {
        let conn = Self::get_connection(app_handle)?;
    ...
    pub async fn cleanup_old_tasks(app_handle: &AppHandle, max_tasks_to_keep: i64) -> Result<i64, String> {
        ...
        if count > 0 {
            ...
            let sql = format!("DELETE FROM batch_tasks WHERE id IN ({})", placeholders);
