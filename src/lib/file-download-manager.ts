...
export class FileDownloadManager {
  ...
  private progressListeners: Map<
    string,
    (progress: number, bytesPerSec: number) => void
  > = new Map();
  ...
  private async setupProgressListener() {
    try {
      ...
      if (typeof window === 'undefined' || !(window as any).__TAURI__) {
        console.log('Not in Tauri environment, skipping progress listener setup');
        return;
      }
  ...
  addDownload(result: TaskResult, taskName?: string): boolean {
    ...
    // 检查是否已经在下载队列中或正在下载
    const existingTask =
      this.downloadQueue.find((t) => t.url === result.imageUrl) ||
      Array.from(this.activeDownloads.values()).find(
        (t) => t.url === result.imageUrl
      );
    ...
    try {
      if (typeof window !== "undefined")
        window.dispatchEvent(
          new CustomEvent("download:enqueued", { detail: { url: task.url } })
        );
    } catch {}
    return true;
  }
  ...
  private processDownloadQueue(): void {
    if (this.downloadQueue.length === 0) {
      ...
      try {
        if (typeof window !== "undefined")
          window.dispatchEvent(new CustomEvent("download:done"));
      } catch {}
      return;
    }
  ...
  private async downloadFile(task: DownloadTask): Promise<void> {
    ...
    try {
      if (typeof window !== "undefined")
        window.dispatchEvent(
          new CustomEvent("download:start", { detail: { url: task.url } })
        );
    } catch {}
    ...
    try {
      ...
      if (!tauriInvoke) {
        ...
        tauriInvoke =
          (tauriObj &&
            ...
            tauriObj.invoke) ||
          (tauriObj?.core &&
            typeof tauriObj.core.invoke === "function" &&
            tauriObj.core.invoke) ||
          undefined;
      }
      ...
      if (tauriInvoke) {
        ...
        const filenameWithDirs =
          (subdirs.length > 0 ? subdirs.join("/") + "/" : "") + task.filename;
        ...
        console.log("[download] tauri invoke result:", {
          savedPath,
          type: typeof savedPath,
        });
        ...
        try {
          if (typeof window !== "undefined")
            window.dispatchEvent(
              new CustomEvent("download:done", {
                detail: { url: task.url, path: savedPath },
              })
            );
        } catch {}
        ...
        setTimeout(async () => {
          try {
            console.log("[download] 开始异步更新本地路径:", {
              taskId: task.id,
              savedPath,
            });
            
            // 使用更高效的批量更新方式
            await this.updateTaskResultAsync(task.id, savedPath, task.url);
            
            console.log("[download] 异步更新本地路径成功:", {
              id: task.id,
              localPath: savedPath,
            });
          } catch (e) {
            console.error("[download] 异步更新本地路径失败", e);
          }
        }, 0);
      ...
      try {
        if (typeof window !== "undefined")
          window.dispatchEvent(
            new CustomEvent("download:done", { detail: { url: task.url } })
          );
      } catch {}
        ...
        if (typeof window !== "undefined")
          window.dispatchEvent(
            new CustomEvent("download:error", {
              detail: { url: task.url, error: task.error },
            })
          );
      } catch {}
  ...
  /**
   * 异步更新任务结果，避免阻塞UI
   * 使用更高效的批量更新方式，减少数据库查询次数
   */
  private async updateTaskResultAsync(
    resultId: string, 
    localPath: string, 
    imageUrl: string
  ): Promise<void> {
    try {
      // 使用 requestIdleCallback 或 setTimeout 确保在空闲时执行
      const updateInIdle = () => {
        this.performTaskResultUpdate(resultId, localPath, imageUrl);
      };

      if (typeof window !== "undefined" && window.requestIdleCallback) {
        window.requestIdleCallback(updateInIdle, { timeout: 1000 });
      } else {
        setTimeout(updateInIdle, 0);
      }
    } catch (error) {
      console.error("[download] 异步更新任务结果失败:", error);
    }
  }

  /**
   * 执行任务结果更新
   * 使用高效的批量更新方法，减少数据库访问次数
   */
  private async performTaskResultUpdate(
    resultId: string, 
    localPath: string, 
    imageUrl: string
  ): Promise<void> {
    try {
      // 使用新的批量更新方法，更高效
      const updated = await sqliteStorage.updateTaskResult(resultId, localPath);
      ...
      if (updated) {
        ...
        console.log("[download] 任务结果更新成功:", {
          resultId,
          localPath,
        });
      } else {
        console.warn("[download] 未找到匹配的 TaskResult:", {
          resultId,
          imageUrl,
        });
      }
    } catch (error) {
      console.error("[download] 更新任务结果失败:", error);
    }
  }
